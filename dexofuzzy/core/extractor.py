...
class ExtractOpcode:
    ...
    def get_opcodes(self, dex_data: bytes) -> list:
        ...
        if (isinstance(dex_data, bytes) and dex_data[0:8] in dex_magic_numbers):
            self.dex = dex_data
            self.__get_header()
    ...
    def __decode_method(self, offset, methods):
        for _ in range(methods):
            ...
            if code_off != 0:
                ...
                self.__get_bytecode(bytecode_off, bytecode_size)
    ...
    def __get_bytecode(self, offset, bytecode_size):
        try:
